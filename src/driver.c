...
static void send_box(spi_device_handle_t spi, int xpos, int ypos, int width, int height, uint16_t *linedata)
{
    esp_err_t ret;
    int x;
...
{
    CAMLparam1(ml_spi_max_transfer_size);

    spi_max_transfer_size = Int_val(ml_spi_max_transfer_size);
...
#else
        .clock_speed_hz=10*1000*1000,           //Clock out at 10 MHz ~ 1MBps
#endif
        .mode=0,                                //SPI mode 0
...
{
    CAMLparam5(ml_buffer, ml_x, ml_y, ml_width, ml_height);
    int x = Int_val(ml_x);
    int y = Int_val(ml_y);
    int width = Int_val(ml_width);
    int height = Int_val(ml_height);

    if (width*height*2 > spi_max_transfer_size) {
        printf("%dx%d vs %d\n", width, height, spi_max_transfer_size);
        caml_failwith("Buffer is too big for a single SPI tranfer.");
    } else {
        uint16_t *buffer = Caml_ba_data_val(ml_buffer);
